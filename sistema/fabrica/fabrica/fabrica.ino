...
void envia_display(uint8_t valor) {
  // Latch LOW
  PORTD &= ~(1 << SRCLK);
...
void setup_display() {
  ...
  DDRB |= (1 << DDB3);     // Configura o pino D11 (PB3) como saída
  DDRB |= (1 << DDB2);     // Configura o pino D10 (PB2) como saída
}
...
void alterna_digito(){
  ...
  send_number(dig2);
  if(trocou){
    PORTB &= ~(1 << PORTB2); // d10 em baixo
    PORTB |= (1 << PORTB3);  // Coloca o pino D11 em nível alto
    envia_display(numeros[dig2]);
    trocou = 0;
  }
  else{
    PORTB &= ~(1 << PORTB3); // d11 em baixo
    PORTB |= (1 << PORTB2);  // Coloca o pino D10 em nível alto
    envia_display(numeros[dig1]);
    trocou = 1;
  }
}
